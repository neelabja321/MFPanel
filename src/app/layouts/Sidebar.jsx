import { NavLink } from 'react-router-dom'
import {
  LayoutDashboard,
  Users,
  CreditCard,
  PiggyBank,
  UsersRound,
  ChevronLeft,
  UserCog,
  ArrowLeftRight,
  Shield,
} from 'lucide-react'
import { useUIStore, useAuthStore } from '@/store'
import { canAccess, MODULES, PERMISSIONS } from '@/lib/accessControl'
import { cn } from '@/lib/utils'

const navItems = [
  { to: '/', icon: LayoutDashboard, label: 'Dashboard', module: MODULES.MASTER },
  { to: '/customers', icon: Users, label: 'Customers', module: MODULES.CUSTOMER },
  { to: '/loans', icon: CreditCard, label: 'Loans', module: MODULES.LOAN },
  { to: '/savings', icon: PiggyBank, label: 'Savings', module: MODULES.DEPOSIT },
  { to: '/groups', icon: UsersRound, label: 'Groups', module: MODULES.GROUP },
  { to: '/transactions', icon: ArrowLeftRight, label: 'Transactions', module: [MODULES.DEPOSIT, MODULES.LOAN] },
  { to: '/users', icon: UserCog, label: 'Users', module: MODULES.ADMINISTRATOR },
  { to: '/roles', icon: Shield, label: 'Roles Matrix', module: MODULES.ADMINISTRATOR },
]

export default function Sidebar() {
  const { sidebarOpen, toggleSidebar } = useUIStore()
  const permissions = useAuthStore((state) => state.permissions)
  const allowedItems = navItems.filter((item) =>
    canAccess(permissions, item.module, PERMISSIONS.LIST))

  return (
    <>
      {sidebarOpen && (
        <div
          className="fixed inset-0 bg-black/50 z-30 md:hidden"
          onClick={toggleSidebar}
        />
      )}
      <aside
        className={cn(
          'absolute md:relative z-40 h-screen bg-sidebar text-sidebar-foreground flex flex-col transition-all duration-300 ease-in-out shrink-0 print:hidden',
          sidebarOpen ? 'w-64 translate-x-0' : '-translate-x-full md:translate-x-0 md:w-16'
        )}
      >
        <div className="h-20 flex items-center gap-4 px-6 shrink-0 pt-2 pb-2">
          <div className="w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center overflow-hidden shrink-0 border border-border">
            <img src="/logo.png" alt="FinanceOrbit Logo" className="w-full h-full object-cover" />
          </div>
          {sidebarOpen && (
            <div className="overflow-hidden flex flex-col justify-center">
              <p className="font-bold text-white text-lg leading-tight tracking-tight">FinanceOrbit</p>
              <p className="text-[11px] text-blue-200/80 font-medium tracking-wide uppercase mt-0.5">Microfinance</p>
            </div>
          )}
        </div>

        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto scrollbar-thin">
          {allowedItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              key={to}
              to={to}
              end={to === '/'}
              className={({ isActive }) => cn(
                'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group',
                isActive
                  ? 'bg-sidebar-primary text-white shadow-sm'
                  : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-white'
              )}
            >
              <Icon className="w-5 h-5 shrink-0" />
              {sidebarOpen && <span className="truncate">{label}</span>}
            </NavLink>
          ))}
          {allowedItems.length === 0 && sidebarOpen && (
            <p className="px-3 py-2 text-xs text-sidebar-foreground/60">No modules assigned</p>
          )}
        </nav>

        <button
          type="button"
          onClick={toggleSidebar}
          className="absolute -right-3 top-20 z-10 flex items-center justify-center w-6 h-6 rounded-full bg-white border border-border shadow-md text-muted-foreground hover:text-primary transition-colors"
          aria-label={sidebarOpen ? 'Collapse sidebar' : 'Expand sidebar'}
        >
          <ChevronLeft
            className={cn('w-3.5 h-3.5 transition-transform duration-300', !sidebarOpen && 'rotate-180')}
          />
        </button>

        <div className="p-4 border-t border-sidebar-border">
          {sidebarOpen ? (
            <p className="text-xs text-sidebar-foreground/40 text-center">v1.0.0 — April 2026</p>
          ) : (
            <div className="w-2 h-2 rounded-full bg-green-400 mx-auto" title="System online" />
          )}
        </div>
      </aside>
    </>
  )
}
